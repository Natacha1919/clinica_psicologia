...
// CORREÇÃO 1: Corrigido o import para 'dart:async'
import 'dart:async';
...
class DashboardService {
  ...
  // CORREÇÃO 2: Método reescrito com a sintaxe atual da API do Supabase
  void _listenToChanges() {
    _pacientesChannel = _supabase
        .channel('public:pacientes_inscritos')
        .onPostgresChanges(
          event: PostgresChangeEvent.all, // Ouve todos os eventos (INSERT, UPDATE, DELETE)
          schema: 'public',
          table: 'pacientes_inscritos',
          callback: (payload) {
            debugPrint('Mudança detectada na tabela de pacientes! Recarregando métricas...');
            _fetchData();
          },
        )
        .subscribe();
