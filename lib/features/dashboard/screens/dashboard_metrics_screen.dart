...
class _DashboardMetricsScreenState extends State<DashboardMetricsScreen> {
          ...
          if (snapshot.hasData) {
            ...
            return RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  // ADICIONADO DE VOLTA: O título da seção que havíamos criado.
  ...
  // ADICIONADO DE VOLTA: O widget que constrói o título da seção.
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildMetricCard(
              "Total de Pacientes",
              metrics.totalPacientes.toString(),
              Icons.people,
              _primaryDark,
            ),
          ],
  ...
  Widget _buildStatusPieChart(StatusCount statusCount) {
    final filteredData = Map.fromEntries(
      statusCount.entries.where((entry) => entry.value > 0),
    );
    ...
    if (filteredData.isEmpty) {
      return Container(
        ...
        child: Text(
          'Nenhum dado de status para exibir',
    ...
    final List<MapEntry<String, int>> dataEntries = [
      ...topEntries,
      if (othersTotal > 0) MapEntry('OUTROS', othersTotal),
    ];

    double total = dataEntries.fold(
      0,
      (sum, entry) => sum + entry.value.toDouble(),
    );

    return Container(
      width: 700, // alterado de 500 para 700
      height: 400,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
                    ...
                    children: List.generate(dataEntries.length, (i) {
                      ...
                      return Padding(
                        ...
                        child: _buildLegendItem(
                          _statusColors[entry.key] ?? Colors.orange.shade400,
  ...
  Widget _buildAgeBarChart(AgeDistribution ageDistribution) {
    final chartData = ageDistribution.entries
        .where((entry) => entry.value > 0)
        .map(
          (entry) =>
              _ChartData(entry.key, entry.value.toDouble(), _accentGreen),
        )
        .toList();
    if (chartData.isEmpty) {
      return Container(
        ...
        child: Text(
          'Nenhum dado de idade para exibir',
          style: TextStyle(color: Colors.grey.shade600),
        ),
      );
    }
